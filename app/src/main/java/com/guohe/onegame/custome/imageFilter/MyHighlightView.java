package com.guohe.onegame.custome.imageFilter;
...
import com.guohe.onegame.custome.imageFilter.drawable.EditableDrawable;
import com.guohe.onegame.custome.imageFilter.drawable.FeatherDrawable;
...
public class MyHighlightView implements EditableDrawable.OnSizeChange {
    ...
    public void setScaleable(boolean scaleable) {
        ...
        if (scaleable) {
            mAnchorRotate = CustomeApplication.getApplication().getResources().getDrawable(R.drawable.aviary_resize_knob);
        } else {
            mAnchorRotate = null;
        }
    }
    ...
    public void setDeleteable(boolean deleteable) {
        if (deleteable) {
            mAnchorDelete = CustomeApplication.getApplication().getResources().getDrawable(R.drawable.aviary_delete_knob);
        } else {
            mAnchorDelete = null;
        }
    }
    ...
    public void draw(final Canvas canvas) {
        ...
        if (is_selected || is_focused) {
            ...
            if (mShowAnchors) {
                ...
                outlinePaint.setStrokeWidth(DimenUtil.dp2px(1));
    ...
    public int getHit(float x, float y) {
        ...
        if (mScaleEnabled) {
            ...
            if ((Math.abs(rect.left - x) < HIT_TOLERANCE) && verticalCheck
                && DynamicUtil.checkBits(mResizeEdgeMode, GROW_LEFT_EDGE)) {
                Log.d(LOG_TAG, "left");
                retval |= GROW_LEFT_EDGE;
            }
            if ((Math.abs(rect.right - x) < HIT_TOLERANCE) && verticalCheck
                && DynamicUtil.checkBits(mResizeEdgeMode, GROW_RIGHT_EDGE)) {
                Log.d(LOG_TAG, "right");
                retval |= GROW_RIGHT_EDGE;
            }
            if ((Math.abs(rect.top - y) < HIT_TOLERANCE) && horizCheck
                && DynamicUtil.checkBits(mResizeEdgeMode, GROW_TOP_EDGE)) {
                Log.d(LOG_TAG, "top");
                retval |= GROW_TOP_EDGE;
            }
            if ((Math.abs(rect.bottom - y) < HIT_TOLERANCE) && horizCheck
                && DynamicUtil.checkBits(mResizeEdgeMode, GROW_BOTTOM_EDGE)) {
    ...
    public void onMouseMove(int edge, MotionEvent event2, float dx, float dy) {
        ...
        if (edge == MOVE) {
            ...
            boolean is_left = DynamicUtil.checkBits(edge, GROW_LEFT_EDGE);
            boolean is_top = DynamicUtil.checkBits(edge, GROW_TOP_EDGE);
    ...
    void rotateBy(final float dx, final float dy, float diffx, float diffy) {
        ...
        final double angle1 = DynamicUtil.angleBetweenPoints(pt2, pt1);
        final double angle2 = DynamicUtil.angleBetweenPoints(pt3, pt1);
        ...
        if (mScaleEnabled) {
            ...
            final double distance1 = DynamicUtil.distance(pt1, pt2);
            final double distance2 = DynamicUtil.distance(pt1, pt4);
