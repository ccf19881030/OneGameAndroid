...
package com.guohe.onegame.update;
...
class UpdateAgent implements ICheckAgent, IUpdateAgent, IDownloadAgent {
    ...
    public UpdateAgent(Context context, boolean isManual, boolean isWifiOnly, int notifyId) {
    ...
    void doCheck() {
        /*new GlobalPresenter().requestCheckUpdate(new Subscriber<BaseResultObjectBean<UpdateInfo>>() {
            @Override
            ...
            public void onError(Throwable e) {
                setError(new UpdateError(UpdateError.CHECK_NETWORK_IO));
            }

            @Override
            public void onNext(BaseResultObjectBean<UpdateInfo> result) {
                String jsonInfo  = new Gson().toJson(result.getData());
                setInfo(jsonInfo);
            }
        });*/
    ...
    void doCheckFinish() {
        ...
        if (error != null) {
            ...
            if (info == null) {
                ...
                mTmpFile = new File(mContext.getExternalCacheDir(), String.valueOf(info.versionCode));
                mApkFile = new File(mContext.getExternalCacheDir(), info.versionCode + ".apk");
                if (UpdateUtil.verify(mApkFile, mInfo.md5)) {
                    doInstall();
